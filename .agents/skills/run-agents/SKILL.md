...
Run the **`financial-analyst`** skill (`.Codex/skills/financial-analyst/SKILL.md`). It applies the
Codex for Financial Services **equity-research** methodology (screen → sector → comps → catalysts →
...
- **Cost:** the swarm is ~$0.07-0.20/run on OpenRouter; the real-MiroFish swarm is the expensive
  ...
  or `--skip mirofish` to leave it out. The analyst runs on the Codex plan.
